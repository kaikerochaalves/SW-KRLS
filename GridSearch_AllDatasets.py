...
l_N = [25, 50, 75, 100, 150, 200, 250, 300]
...
columns = ['N', 'sigma', 'c', 'RMSE', 'NDEI', 'MAE']
result = pd.DataFrame(columns = columns)
for N in l_N:
    for sigma in l_sigma:
        for c in l_c:
                            
            print(f"N = {N} and sigma = {sigma} and c = {c}")
        
            # Initializing the model
            model = SW_KRLS(N = N, sigma = sigma, c = c)
            # Train the model
            ...
            NewRow = pd.DataFrame([[N, sigma, c, RMSE, NDEI, MAE]], columns = columns)
